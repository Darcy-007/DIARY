...
struct OnboardingView: View {
    ...
    @State private var transactionStatus: AuthorizationStatus?
        ...
        VStack(spacing: 32) {
            ...
            case 3:
                permissionStep(
                    ...
                    description: "dAIry reads your Apple Pay transactions to reflect your spending in diary entries.",
    ...
    private var welcomeStep: some View {
        VStack(spacing: 16) {
            ...
            Text("Welcome to dAIry")
                .font(.largeTitle.bold())
            Text("Your AI-powered daily diary. dAIry collects photos, health data, and transactions to write a personal narrative of your day.")
                .font(.body)
    ...
    private func permissionStep(
        ...
        VStack(spacing: 16) {
            ...
            if let status {
                statusView(for: status, source: dataSourceForStep)
            }
        }
    }
        ...
        case .denied, .revoked:
            VStack(spacing: 8) {
                Label("Access Denied", systemImage: "xmark.circle.fill")
                    .foregroundStyle(.red)
                    .font(.headline)
                Text(PermissionManager.denialMessage(for: source))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        case .notDetermined:
    ...
    private var bottomButton: some View {
        ...
        case 1:
            if photoStatus == nil {
                requestButton(title: "Allow Photo Access") {
                    photoStatus = await permissionManager.requestPhotoAccess()
                }
            } else {
        ...
        case 2:
            if healthStatus == nil {
                requestButton(title: "Allow Health Access") {
                    healthStatus = await permissionManager.requestHealthKitAccess()
                }
            } else {
        ...
        case 3:
            if transactionStatus == nil {
                requestButton(title: "Allow Transaction Access") {
                    transactionStatus = await permissionManager.requestTransactionAccess()
                }
            } else {
                continueButton { currentStep = 4 }
            }

        case 4:
            // Task 10.3 — API key step
            VStack(spacing: 12) {
                Button {
                    completeOnboarding()
                } label: {
